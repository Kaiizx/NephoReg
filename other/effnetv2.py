...
# Add the MobileNet base model (excluding the top layers)
base_model = EfficientNetV2M(include_top=False, input_shape=(224, 224, 3))
...
checkpoint = ModelCheckpoint("/home/dip_21/project/checkpoint/effnetv2.h", monitor='val_acc', verbose=1, save_best_only=True, save_weights_only=False)
early = EarlyStopping(monitor='val_acc', patience=8, verbose=1)
start_time = time.time()
resnet_history = model.fit(train_generator, validation_data = validation_generator, 
                           steps_per_epoch = step_size_train, 
                           validation_steps = step_size_val,
                           class_weight=class_weights,
                           epochs = 150, callbacks=[checkpoint,early,wandb_callback])
end_time = time.time()
...
plt.legend()
plt.grid(True)
plt.savefig(f'/home/dip_21/project/plot/acc_effnet.png')
...
plt.legend()
plt.grid(True)
plt.savefig(f'/home/dip_21/project/plot/loss_effnet.png')
